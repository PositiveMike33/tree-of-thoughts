# thought -> evaluated value (0.4, This solution is invalid because x) -> thought
# prompt + this solution is invalid because + better eval
...
class TreeofThoughts:
    def __init__(self, model, export_to_obsidian: bool = False, vault_path: str = None):
        self.model = model
        ...
        self.export_to_obsidian = export_to_obsidian
        self.vault_path = vault_path
        self.obsidian_adapter = None

        # Initialize Obsidian exporter if enabled
        if self.export_to_obsidian:
            try:
                from tree_of_thoughts.exporters.obsidian_adapter import ObsidianAdapter

                self.obsidian_adapter = ObsidianAdapter(
                    output_dir="./logs", vault_path=vault_path
                )
            except ImportError:
                logger.warning(
                    "Could not import ObsidianAdapter. Obsidian export disabled."
                )
    ...
    def save_tree_to_json(self, file_name):
        ...
        # Automatically export to Obsidian if enabled
        if self.export_to_obsidian and self.obsidian_adapter:
            self._export_to_obsidian(file_name)

    def _export_to_obsidian(self, json_file_path: str = None) -> None:
        """Export tree data to Obsidian format.

        Args:
            json_file_path: Optional path to JSON file for reference.
        """
        ...
        try:
            metadata = {
                ...
                "model": (
                    str(self.model.__class__.__name__) if self.model else "unknown"
                ),
                "best_state": str(self.best_state) if self.best_state else "none",
                "best_value": (
                    float(self.best_value) if self.best_value != float("-inf") else 0
                ),
            }

            # Export to Obsidian format
            output_path = self.obsidian_adapter.export(
                tot_data=self.tree, metadata=metadata
            )

            logger.info(f"Exported to Obsidian: {output_path}")
