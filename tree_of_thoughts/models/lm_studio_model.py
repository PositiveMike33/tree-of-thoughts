...
class LMStudioLanguageModel(AbstractLanguageModel):
    """Language model using LM Studio (local models)"""

    def __init__(self, api_url="", model="local-model", max_tokens=1024, temperature=0.7,
                 top_p=1, strategy="cot", evaluation_strategy="value", enable_ReAct_prompting=True):
        if api_url == "" or api_url is None:
            api_url = os.environ.get("LM_STUDIO_API_URL", "http://localhost:1234")
        ...
        logger.info(f"Initialized LMStudioLanguageModel with model: {self.model} at {self.api_url}")

    def lm_studio_api_call(self, prompt, max_tokens=None, temperature=None):
        """Call LM Studio API"""
    ...
    def generate_text(self, prompt, k):
        ...
        for _ in range(k):
            try:
                response = self.lm_studio_api_call(prompt)
    ...
    def evaluate_states(self, states, initial_prompt):
        ...
        if self.evaluation_strategy == 'value':
            ...
            for state in states:
                ...
                try:
                    response = self.lm_studio_api_call(prompt, max_tokens=10, temperature=1)
            ...
            try:
                response = self.lm_studio_api_call(prompt, max_tokens=50, temperature=1)
