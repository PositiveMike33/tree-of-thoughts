...
class DeepSeekLanguageModel(AbstractLanguageModel):
    """Language model using DeepSeek API"""

    def __init__(self, api_key="", model="deepseek-chat", max_tokens=1024, temperature=0.7,
                 top_p=1, strategy="cot", evaluation_strategy="value", enable_ReAct_prompting=True):
        if api_key == "" or api_key is None:
            api_key = os.environ.get("DEEPSEEK_API_KEY", "")
        if api_key == "":
            raise Exception("Please provide DeepSeek API key via DEEPSEEK_API_KEY environment variable")
        ...
        logger.info(f"Initialized DeepSeekLanguageModel with model: {self.model}")

    def deepseek_api_call(self, prompt, max_tokens=None, temperature=None):
        """Call DeepSeek API"""
    ...
    def evaluate_states(self, states, initial_prompt):
        ...
        if self.evaluation_strategy == 'value':
            ...
            for state in states:
                ...
                try:
                    response = self.deepseek_api_call(prompt, max_tokens=10, temperature=1)
            ...
            try:
                response = self.deepseek_api_call(prompt, max_tokens=50, temperature=1)
